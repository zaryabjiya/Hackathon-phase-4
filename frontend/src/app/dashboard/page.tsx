...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('chatbot-task-update', handleTaskUpdate);
    
    return () => {
      window.removeEventListener('chatbot-task-update', handleTaskUpdate);
    };
  }, [fetchTasks]);
  ...
  const handleTaskDelete = async (taskId: number) => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL || 'http://127.0.0.1:8000'}/users/${user!.id}/tasks/${taskId}`, {
        method: 'DELETE',
        headers: {
