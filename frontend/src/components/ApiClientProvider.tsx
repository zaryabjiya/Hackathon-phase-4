...
interface ApiClientContextType {
  ...
  chat: (userId: string, message: string, conversationId?: string) => Promise<any>;
}
...
export const ApiClientProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const chat = async (userId: string, message: string, conversationId?: string) => {
    return request(`/api/${userId}/chat`, {
      method: 'POST',
      body: JSON.stringify({
        message,
