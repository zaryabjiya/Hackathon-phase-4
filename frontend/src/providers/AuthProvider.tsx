...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  const login = async (credentials: { email: string; password: string }) => {
    ...
    try {
      ...
      // Re-throw with more helpful message for network errors
      if (error.message.includes('Failed to fetch') || error.name === 'TypeError') {
        throw new Error(`Unable to connect to the authentication server. Please ensure the backend is running at ${API_BASE_URL}`);
      }
      throw error;
    } finally {
      setLoading(false);
  ...
  const register = async (userData: { email: string; username: string; password: string }) => {
    ...
    try {
      ...
      // Re-throw with more helpful message for network errors
      if (error.message.includes('Failed to fetch') || error.name === 'TypeError') {
        throw new Error(`Unable to connect to the registration server. Please ensure the backend is running at ${API_BASE_URL}`);
      }
      throw error;
    } finally {
      setLoading(false);
